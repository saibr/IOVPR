...
from ibl.faiss_evaluators import Evaluator, extract_features_q, extract_features_db, pairwise_distance
...
def main_worker(args):
    ...
    # Create data loaders
    dataset, ams_train, train_extract_loader, test_loader_q, test_loader_db = get_data(args)
    ...
    if (args.reduction):
        ...
        if (not osp.isfile(pca_parameters_path)):
            dict_f = extract_features_db(model, train_extract_loader, pitts_train, features_path = args.features_path, features_name = args.features_name, vlad=args.vlad, gpu=args.gpu, sync_gather=args.sync_gather)
            dict_f = extract_features_db(model, train_extract_loader, pitts_train, vlad=args.vlad, gpu=args.gpu,
    ...
    recalls, viz = evaluator.evaluate(test_loader_q, sorted(list(set(dataset.q_test) | set(dataset.db_test))),
                    dataset.q_test, dataset.db_test, dataset.test_pos, gallery_loader=test_loader_db,
                    features_path=args.features_path, features_name = args.features_name,
                    vlad=args.vlad, pca=pca, rerank=args.rerank, gpu=args.gpu, sync_gather=args.sync_gather,
                    nms=(True if args.dataset=='tokyo' else False),rr_topk=args.rr_topk, lambda_value=args.lambda_value)

    if (args.rank==0):
        np.savez("results_"+osp.basename(args.resume).split('.')[0]+"_"+args.dataset+"_"+args.scale+"_"+args.resume.split('/')[7]+"_"+args.backbone+".npz",
            recalls=recalls, viz=viz, queries=dataset.q_test)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--features_path', type=str, metavar='PATH',
                        default='')
